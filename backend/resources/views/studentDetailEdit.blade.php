...
@section('content')
    ...
    <form class="input-group mb-3 w-25 d-flex flex-column mx-auto" method="post" action="{{route('studentUpdate',['id' => $id])}}">
        @csrf
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input name="math" type="number" class="form-control" min="0" max="10" value="{{$gradeMath['grade'] ?? 0}}">
        </div>
        <div class="mb-3">
            <label class="form-label">Music</label>
            <input name="music" type="number" class="form-control" min="0" max="10" value="{{$gradeMusic['grade'] ?? 0}}">
        </div>
        <div class="mb-3">
            <label class="form-label">English</label>
            <input name="english" type="number" class="form-control" min="0" max="10" value="{{$gradeEnglish['grade'] ?? 0}}">
        </div>
        <div class="d-flex justify-content-center gap-2 px-5 py-2">
            <button type="submit" class="btn btn-success d-flex align-items-center">Save</button>
        </div>
    </form>
    @if(session('success'))
